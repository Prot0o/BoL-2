...
local  IreliaSmgStyle_version = 1
...
if Settings.AutoUlt.UseAutoW then
	for _, unit in pairs(GetEnemyHeroes()) do
			local rPos, HitChance, maxHit, Positions = VP:GetConeAOECastPosition(unit, SkillW.delay, 40, SkillW.range, SkillW.speed, myHero, false)	
			if ValidTarget(unit, SkillW.range) and rPos ~= nil and maxHit >= Settings.AutoUlt.ARX then
					Packet("S_CAST", {spellId = _W, fromX = rPos.x, fromY = rPos.z, toX = rPos.x, toY = rPos.z}):send()
			end
		end
	end
end
...
function Menu()
	Settings = scriptConfig("Irelia - SmG Style", "Despe")
	...
	Settings:addSubMenu("["..myHero.charName.."] - Auto W ", "AutoUlt")
		Settings.AutoUlt:addParam("UseAutoW", "Auto E if X unite", SCRIPT_PARAM_ONKEYTOGGLE, false, GetKey("V"))
	...
	Settings:addSubMenu("["..myHero.charName.."] - Orbwalking Settings", "Orbwalking")
		...
		Settings.AutoUlt:permaShow("UseAutoW")
